...
class Evaluation extends CI_Controller{
		...
		if($this->input->post()){
			...
			switch ($currentStep) {
				...
				case 2:
					...
					//if dili ma ansiran tanan
					//check_answer_section
					if(sizeof($answers) != 9){
						for ($i=0; $i < 9; $i++) {
							if(!isset($answers[$i])){
								$answers[$i] = -1;
					...
					//end check_answer_section

					//studentCode ang gigamit og checking if student ba ang ni evaluate or academic head
					//kung studentCode not equal to null, tapos ang type ky acad, acad jud sya
					if($evaluation['studentCode']!=''){
					  	if($evaluation['type']=='Student'){
				...
				case 3:
					...
					$evaluation['admin_responsibilities'] = $answers;
				...
				case 4:
					...
					$evaluation['instruc_responsibilities'] = $instrucAnswer;
					...
					if($evaluation['studentCode']!=''){
					  	if($evaluation['type']=='Student'){
							redirect("/evaluation/start?type=Student&uid=$uid&step=5");
						}else{
							redirect("/evaluation/start?type=Academic&uid=$uid&step=5");
						}
					}else{
							redirect("/evaluation/start?type=Faculty&uid=$uid&step=5");
					}
				...
				case 5:
					...
					$instrucAnswer = $evaluation['instruc_responsibilities'];
	...
	//	$type = $data['type'];
		$this->output->set_template('default');
		//note: gi comment ang checking sa type ky ang student, facutly itself og acad pareha ra og questioner. that's why no need checking
		//if($type=='Student'){
	...
	function ShowStepFive($data){
		...
		//note: gi comment ang checking sa type ky ang student, facutly itself og acad pareha ra og questioner. that's why no need checking
	//	if($type=='Student'){
		$this->load->view("evaluation_summary.html",$data);
	//}elseif($type=='Academic'){
	//	$this->load->view("academic_head_form2.html",$data);
	//}else{
